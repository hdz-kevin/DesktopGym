...
class TestCrearMembresia:
    ...
    def test_copia_el_precio_de_la_duracion(self, app_db, app_catalog):
        member_id = alta()
        membership_id = service.create_membership(member_id, app_catalog["monthly_id"])
    ...
    def test_calcula_el_vencimiento_segun_la_duracion(self, app_db, app_catalog):
        member_id = alta()
    ...
    def test_duracion_inexistente(self, app_db):
        with pytest.raises(NotFoundError):
...
class TestCatalogoDePrecios:
    def test_lista_los_tipos_con_sus_duraciones(self, app_db, app_catalog):
        tipos = service.list_membership_types()
        assert len(tipos) == 1
        assert len(tipos[0].durations) == 2

    def test_crea_un_tipo(self, app_db, app_catalog):
        service.create_membership_type("Premium")
        assert len(service.list_membership_types()) == 2

    def test_no_admite_tipos_duplicados(self, app_db, app_catalog):
        with pytest.raises(ValidationError):
            service.create_membership_type("general")

    def test_rechaza_nombre_vacio(self, app_db):
        with pytest.raises(ValidationError):
            service.create_membership_type("   ")

    def test_renombra_un_tipo(self, app_db, app_catalog):
        service.rename_membership_type(app_catalog["type_id"], "General Plus")
        assert service.list_membership_types()[0].name == "General Plus"

    def test_no_borra_un_tipo_en_uso(self, app_db, app_catalog):
        service.create_membership(alta(), app_catalog["monthly_id"])
        with pytest.raises(ServiceError, match="membresías registradas"):
            service.delete_membership_type(app_catalog["type_id"])

    def test_borra_un_tipo_sin_uso(self, app_db):
        type_id = service.create_membership_type("Temporal")
        service.delete_membership_type(type_id)
        assert all(t.id != type_id for t in service.list_membership_types())

    def test_crea_una_duracion(self, app_db, app_catalog):
        service.create_duration(app_catalog["type_id"], "Trimestral", 3, DurationUnit.MONTH, 100000)
    ...
    def test_valida_las_duraciones(self, app_db, app_catalog, nombre, cantidad, precio, campo):
        with pytest.raises(ValidationError) as exc:
            service.create_duration(
                app_catalog["type_id"], nombre, cantidad, DurationUnit.MONTH, precio
            )
        assert campo in exc.value.errors

    def test_actualiza_una_duracion(self, app_db, app_catalog):
        service.update_duration(app_catalog["monthly_id"], "Mensual", 1, DurationUnit.MONTH, 45000)
        mensual = next(d for d in service.list_durations() if d.id == app_catalog["monthly_id"])
    ...
    def test_subir_el_precio_no_reescribe_lo_ya_cobrado(self, app_db, app_catalog):
        ...
        service.update_duration(app_catalog["monthly_id"], "Mensual", 1, DurationUnit.MONTH, 90000)

        periodo = service.get_membership(membership_id).periods[0]
        assert periodo.price_paid_cents == 40000

    def test_no_borra_una_duracion_usada(self, app_db, app_catalog):
        service.create_membership(alta(), app_catalog["monthly_id"])
        with pytest.raises(ServiceError, match="periodos ya registrados"):
            service.delete_duration(app_catalog["monthly_id"])

    def test_borra_una_duracion_sin_uso(self, app_db, app_catalog):
        duration_id = service.create_duration(
            app_catalog["type_id"], "Anual", 12, DurationUnit.MONTH, 400000
        )
        service.delete_duration(duration_id)
        assert all(d.id != duration_id for d in service.list_durations())
